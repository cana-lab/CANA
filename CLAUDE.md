...
## Source map (src/)
- `App.jsx` (~3400 lines) — all UI. Screens are gated `if (screen==="x")`:
  ...
  Info modals share the `InfoModal` shell + `pressable` chrome-less button style.
- `engine.js` (~1380 lines) — DETERMINISTIC core. 11 domains, 115 questions,
...
- `nativePlugins.js` — registerPlugin() proxies for the three app-local iOS
  plugins + `isNativeIOS()` + the startup console diagnostic
  (`[CANA] native plugins: ShareFile=true …`). See the THREE-part registration
  note under "iOS project layout".
- `credentials.js` — unified "remember password" wrapper: Electron safeStorage
  bridge on Mac, Credentials plugin (iOS Keychain) on iPhone, off on web.
- `crashLog.js` — local rolling crash/error log (25 entries, device-only).
...
- **App-local Capacitor plugins need THREE pieces** (learned over 4.54.2 →
  4.55.2): JS `registerPlugin()` (src/nativePlugins.js), Swift `CAPBridgedPlugin`
  conformance (method metadata), AND runtime registration in
  `MainViewController.capacitorDidLoad()` via `registerPluginInstance` (wired
  through Main.storyboard). Without the third piece every call rejects with
  "plugin is not implemented on ios" — and a green compile proves NOTHING,
  because registration is runtime-only. Verify by launching in the simulator
  and grepping the console for the `[CANA] native plugins:` line.
