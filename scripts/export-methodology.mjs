...
P();
P(`### Calibrations introduced in v4.50 (all transparent, never silent)`);
P();
P(`11. **Per-item weights.** Domain means are weighted: appreciation (m9)`);
P(`    carries w=2 inside Marriage (top-tier predictor of relationship`);
P(`    quality); the three couple-level spiritual items f8/f12/f13 carry 70%`);
P(`    of the Faith domain's internal mass vs 30% for the eleven individual/`);
P(`    congregational items (proximal vs distal religiousness, Mahoney).`);
P(`12. **Threefold Cord bonus.** If the couple-mean of f8 (sanctification)`);
P(`    and m8 (shared prayer) averages ≥ 7.5, the Marriage score gets ×1.2,`);
P(`    capped at 10 — disclosed by a "Threefold Cord" flag carrying the`);
P(`    unadjusted value (shared sanctification as conflict buffer).`);
P(`13. **Spiritual-divergence calibration.** A Faith gap ≥ 3.0 applies a 15%`);
P(`    adjustment to the shared Faith score — disclosed in the "Spiritual`);
P(`    Misalignment" flag. Per-partner views and the gap itself stay raw.`);
P(`14. **Oxygen Check (suffocation model).** If couple-mean expectations`);
P(`    (f12, m16, m5) ≥ 8.0 while couple-mean resources (v3, b7, m4) ≤ 4.0,`);
P(`    a "Resource/Expectation Imbalance" flag fires. No score impact.`);
P(`15. **A practice for this season.** One band-based recommendation:`);
P(`    Thriving (≥8) → 7-minute conflict reappraisal 3×/year (Finkel 2013);`);
P(`    Developing (5–7.9) → 90 min/week novel shared activity (Aron) + daily`);
P(`    benevolent prayer for the partner (Fincham & Beach); At-Risk (<5) →`);
P(`    relationship-movie-and-talk (Rogge 2013). SAFETY: severe self-reported`);
P(`    contempt (raw m15 ≥ 7, either partner) replaces all self-help with a`);
P(`    pastor/counselor referral recommendation.`);
P(`16. **Investment Model coverage.** m19 (investment size) and m20`);
P(`    (commitment / future we-ness) were added. A direct "quality of`);
P(`    alternatives" item was deliberately NOT added: it primes`);
P(`    contract-thinking against the instrument's covenant framing, and the`);
P(`    commitment construct carries the same predictive content positively.`);
P();
